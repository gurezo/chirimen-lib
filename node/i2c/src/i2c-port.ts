...
export class I2CPort {
  ...
  async open(slaveAddress: I2CSlaveAddress): Promise<I2CSlaveDevice> {
    ...
    return {
       ...
       */
      write8: async (registerNumber: number, byte: number) => {
        try {
       ...
       */
      write16: async (registerNumber: number, word: number) => {
        try {
